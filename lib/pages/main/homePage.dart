              ...
              children: [
                ...
                Consumer<products>(builder: (consumer, products, child) {
                  if (products.eventProductList.isEmpty) {
                    return Container(alignment: Alignment.center, height: 250, margin: Constants.SCREEN_HORIZONTAL_MARGIN, child: CircularProgressIndicator());
                  } else {
                    return Container(
                      margin : EdgeInsets.only(left: Constants.SCREEN_HORIZONTAL_MARGIN.horizontal / 2),
                      height: 230,
                      child: ListView.separated(
                        scrollDirection: Axis.horizontal,
                        itemCount: ((products.eventProductList.length < 5) ? products.eventProductList.length : 5) + 1, // 길이 + 1 ( 여백 )
                        itemBuilder: (context, index) {
                          if(index == ((products.eventProductList.length < 5) ? products.eventProductList.length : 5)){
                            return SizedBox( // 마지막 요소 여백
                              width: Constants.SCREEN_HORIZONTAL_MARGIN.horizontal / 2,
                            );
                          }
                          else{
                        ...
                        separatorBuilder: (context, index) => const SizedBox( // 카드 사이 여백
                          width: 10,
                        ),
                      ),
                    );
